import { ComponentProps } from 'react'
import { Friend, SocialProfile } from 'src/components'
import { elon, orion, github, instagram, mark, jeff, linkedin } from './b64'

type Profile = ComponentProps<typeof SocialProfile>

export const profiles: Profile[] = [
  {
    label: 'GitHub',
    url: 'https://github.com/AMACAFELLA/',
    src: github,
  },
  {
    label: 'LinkedIn',
    url: 'https://www.linkedin.com/in/angus-macapella/',
    src: linkedin,
  },
  {
    label: 'Instagram',
    url: 'https://www.instagram.com/macafella_/',
    src: instagram,
  },
]

type Friends = ComponentProps<typeof Friend>[]

export const friends: Friends = [
  {
    src: orion,
    name: 'Orion Macapella',
    label: 'Former Child Star',
    url: 'https://www.linkedin.com/in/orionmac/',
  },
  {
    src: jeff,
    name: 'Jeff Bazos',
    label: '',
    url: 'https://www.google.com/search?q=jeff+bezos&sca_esv=601415381&sxsrf=ACQVn09MBlnP9APW4R3S_4cmIvW_QZteWQ%3A1706191309951&ei=zWmyZcnPOYWrhbIP6t6N6AY&gs_ssp=eJzj4tTP1TcwNKwyszRg9OLKSk1LU0hKrcovBgBIPga9&oq=jeff&gs_lp=Egxnd3Mtd2l6LXNlcnAiBGplZmYqAggAMgoQLhiABBiKBRgnMg0QABiABBiKBRhDGLEDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMgoQABiABBiKBRhDMhcQLhiABBiKBRiXBRjcBBjeBBjfBNgBAkj4ElD_BljvC3ABeAGQAQCYAbwCoAHACaoBBTItMS4zuAEByAEA-AEBqAIRwgIHEC4Y6gIYJ8ICBxAjGOoCGCfCAhYQLhiABBiKBRhDGMgDGOoCGLQC2AEBwgIWEC4YQxiABBiKBRjIAxjqAhi0AtgBAcICChAjGIAEGIoFGCfCAgoQLhiABBiKBRhDwgILEAAYgAQYsQMYgwHCAg4QABiABBiKBRixAxiDAcICDRAuGIAEGIoFGEMYsQPiAwQYACBBugYGCAEQARgIugYGCAIQARgU&sclient=gws-wiz-serp',
  },
  {
    src: mark,
    name: 'Mark Zuckerberg',
    label: '',
    url: 'https://www.google.com/search?q=mark+zuckerberg&sca_esv=601415381&sxsrf=ACQVn0_htCyKsvIUI6ZenB52Dm6rvSoW8g%3A1706191279591&ei=r2myZaTaI_TYhbIP1JWMwAI&gs_ssp=eJzj4tTP1TewMEvJqzRg9OLPTSzKVqgqTc5OLUpKLUoHAHpvCVE&oq=mark+&gs_lp=&sclient=gws-wiz-serp',
  },
  {
    src: elon,
    name: 'Elon Musk',
    label: '',
    url: 'https://www.google.com/search?q=elon+musk&sca_esv=601415381&sxsrf=ACQVn0-qmCptAOINmV6PaiH9TwCCm5_1Xg%3A1706191274756&source=hp&ei=qmmyZf2gLLivhbIPvdaKsA4&iflsig=ANes7DEAAAAAZbJ3ujVfaDWQNtCVve_Z1QDQwvzbClzM&gs_ssp=eJzj4tTP1TcwzqtKMzRg9OJMzcnPU8gtLc4GAEjMBtM&oq=Elon+&gs_lp=&sclient=gws-wiz',
  },
]
